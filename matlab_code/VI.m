# simulation d'un GJR(1,1) avec bruit iid gamma(shape=3,scale=1/2\sqrt{3})
gjr.sim<- function (n, omega,  alpha1, alpha2, beta,df=7) {
...
for (t in 2:n) { 
ht[t] <- omega+alpha1*(pmax(0,eps[t-1]))**2+alpha2*(pmin(0,eps[t-1]))**2+beta*ht[t-1]
...
alpha10<-alphavec[i]
for (j in 1:nrep) {
ser<-gjr.sim(n, omega0,  alpha10, alpha20, beta0)
